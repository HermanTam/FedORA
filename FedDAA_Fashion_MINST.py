...
def run_experiment(args_, exp_logger=None):
    ...
    # Initialize per-client test history if requested
    ...
    for c in clients:
        if getattr(c, 'objective', 'G') == 'P':
            a = evaluate_iterator_accuracy(c.learners_ensemble, c.test_iterator)
            if a is not None:
                objective_aware_old_scores.append(a)
                objective_aware_global_scores.append(a)
        else:
            a_old = evaluate_iterator_accuracy(c.learners_ensemble, getattr(c, 'last_test_iterator', None))
            if a_old is not None:
                objective_aware_old_scores.append(a_old)
            if 'global_test_iterator' in locals() and global_test_iterator is not None:
                a_g = evaluate_iterator_accuracy(c.learners_ensemble, global_test_iterator)
                if a_g is not None:
                    objective_aware_global_scores.append(a_g)
    ...
    if getattr(args_, 'eval_all_past_concepts', False):
        ...
        ap_g = []
        for c in clients:
            hist = getattr(c, '_test_history', None)
            if not hist:
                continue
            accs = []
            for it in hist:
            ...
            if getattr(c, 'objective', 'G') == 'P':
                ...
                ap_g.append(m)
        all_past_stats = {
            'all_clients': sample_stats(ap_all),
            'P_clients': sample_stats(ap_p),
            'G_clients': sample_stats(ap_g),
        }
    ...
    if exp_logger is not None:
        ...
        with open(objective_summary_path, 'w') as f:
            ...
            # Per-objective breakdowns (local/global)
            # Compute P_on_local/G_on_local and P_on_global/G_on_global
            p_on_local_scores = []
            g_on_local_scores = []
            p_on_global_scores = []
            g_on_global_scores = []
            for client in clients:
                # Local
                acc_local = evaluate_iterator_accuracy(client.learners_ensemble, client.test_iterator)
                if acc_local is not None:
                    if getattr(client, 'objective', 'G') == 'P':
                        p_on_local_scores.append(acc_local)
                    else:
                        g_on_local_scores.append(acc_local)
                # Global
                if global_test_iterator is not None:
                    acc_global = evaluate_iterator_accuracy(client.learners_ensemble, global_test_iterator)
                    if acc_global is not None:
                        if getattr(client, 'objective', 'G') == 'P':
                            p_on_global_scores.append(acc_global)
                        else:
                            g_on_global_scores.append(acc_global)
            pol = sample_stats(p_on_local_scores)
            gol = sample_stats(g_on_local_scores)
            pog = sample_stats(p_on_global_scores)
            gog = sample_stats(g_on_global_scores)
            ...
            # Objective-aware variants (suppress mixed to avoid confusion)
